 ...
 * 
 */
class PortletPageListenerInvoker extends PortletInvoker {
	...
	protected void invokeMethod(PortletMethod method, PortletWindowEx window,
		...
		if (portlet instanceof PortletPageListener) {
			if (method == PortletMethod.BEGIN_PAGE) {
				((PortletPageListener) portlet).beginPage(portletRequest, portletResponse);
			} else if (method == PortletMethod.END_PAGE) {
				((PortletPageListener) portlet).endPage(portletRequest, portletResponse);
			}
		}
	}
