 ...
 * 
 */
class PortletSessionListenerInvoker extends PortletInvoker {
	...
	protected void invokeMethod(PortletMethod method, PortletWindowEx window,
		...
		if (portlet instanceof PortletSessionListener) {
			if (method == PortletMethod.LOGIN) {
				((PortletSessionListener) portlet).login(portletRequest);
			} else if (method == PortletMethod.LOGOUT) {
				((PortletSessionListener) portlet).logout(portletRequest.getPortletSession());
			}
		}
	}
