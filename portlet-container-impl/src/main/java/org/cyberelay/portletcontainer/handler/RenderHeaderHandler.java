 ...
 * 
 */
class RenderHeaderHandler extends AbstractHandler {
	...
	protected RenderRequest createRenderRequest(PortletInvocationRequest request, RenderResponse response) {
		RenderRequest req = new RenderRequestImpl(request, response);
		req.setAttribute(RenderRequest.RENDER_PART, RenderRequest.RENDER_HEADERS);
		return req;
	}
}
